...
def compute_objectness_loss(end_points):
    ...
    objectness_score = end_points['objectness_score'] # torch.Size([4, 2, 20000])
    objectness_label = end_points['objectness_label'] # torch.Size([4, 20000])
    loss = criterion(objectness_score, objectness_label)
    end_points['B: Objectness Loss'] = loss

    objectness_pred = torch.argmax(objectness_score, 1) # 计算是物体表面的采样点的物体精度
...
def compute_graspness_loss(end_points):
    ...
    graspness_score = end_points['graspness_score'].squeeze(1) # torch.Size([4, 20000])
    graspness_label = end_points['graspness_label'].squeeze(-1) # torch.Size([4, 20000])
    loss_mask = end_points['objectness_label'].bool() # torch.Size([4, 20000])
...
def compute_view_graspness_loss(end_points):
    ...
    view_score = end_points['view_score'] # torch.Size([4, 1024, 300])
    view_label = end_points['batch_grasp_view_graspness'] # torch.Size([4, 1024, 300])
...
def compute_angle_loss(end_points):
    ...
    grasp_angle_pred = end_points['grasp_angle_pred'] # torch.Size([4, 13, 1024])
    grasp_angle_label = end_points['batch_grasp_rotations'].long() # torch.Size([4, 1024])
    valid_mask = end_points['batch_valid_mask'] # torch.Size([4, 1024])
...
def compute_depth_loss(end_points):
    criterion = nn.CrossEntropyLoss(reduction='none')
    grasp_depth_pred = end_points['grasp_depth_pred'] # torch.Size([4, 5, 1024])
    grasp_depth_label = end_points['batch_grasp_depth'].long() #torch.Size([4, 1024])
    valid_mask = end_points['batch_valid_mask'] # torch.Size([4, 1024])
...
def compute_score_loss_cls(end_points):
    criterion = nn.CrossEntropyLoss(reduction='none')
    grasp_score_pred = end_points['grasp_score_pred'] # torch.Size([4, 6, 1024])
    grasp_score_label = (end_points['batch_grasp_score'] * 10 / 2).long() # torch.Size([4, 1024])
    valid_mask = end_points['batch_valid_mask'] # torch.Size([4, 1024])
...
def compute_width_loss(end_points):
    criterion = nn.SmoothL1Loss(reduction='none')
    grasp_width_pred = end_points['grasp_width_pred'] # torch.Size([4, 1, 1024])
    grasp_width_label = end_points['batch_grasp_width'] * 10 # torch.Size([4, 1024])
    valid_mask = end_points['batch_valid_mask'] # torch.Size([4, 1024])
